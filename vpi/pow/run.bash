...
type gcc
gcc $M32 -c -fpic pow_vpi.c -o pow_vpi.o
gcc $M32 -shared -o libvpi.so pow_vpi.o
realpath libvpi.so
...
if [ $QUESTA ]; then
vlog dut.v
vsim -batch -voptargs=+acc=npr -pli /home/runner/libvpi.so work.tb_laod_counter -do "run -all"
fi

if [ $RIVIERA ]; then
vlib work
vlog dut.v -pli /home/runner/libvpi.so
vsim +access +w_nets -interceptcoutput -c -pli /home/runner/libvpi.so -do "run -all" tb_load_counter
fi
